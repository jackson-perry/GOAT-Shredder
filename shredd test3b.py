...
outputname=docxfile[:-4]+'xlsx'
df.to_excel(filename[:-4]+'xlsx')  # creates an excel file containing the data frame  
##########################   create excel sheet with date and CONOP number  ###############################
wb = load_workbook(XLfilename)  #click on the excel templae from jllis
wb['Sheet1']._tables=[] #the table gets corrupted everytime so this removes it from the template
TABLE=Table(displayName='Table1', ref='A6:AI206') #this adds a new table back in 
STYLE= TableStyleInfo(name='TableStyleMedium2', showFirstColumn=False, showLastColumn=False, showRowStripes=True, showColumnStripes=False) #style for the new table
...
while ROW >=0: # check if we have any observations left
        ...
        if df.iloc[ROW]['TOI Portion Mark']== '(U//FOUO)':
        ...
        if df.iloc[ROW]['Obs Portion Mark']== '(U//FOUO)':
            wb['Sheet1']['F'+str(ROW+7)]='FOUO' 
            wb['Sheet1']['G'+str(ROW+7)]='USA'
            wb['Sheet1']['D1']='FOUO' 
        elif    df.iloc[ROW]['Obs Portion Mark']== 'U': 
        ...
        if df.iloc[ROW]['Discussion Portion Mark']=='(U//FOUO)':
            wb['Sheet1']['J'+str(ROW+7)]='FOUO' 
            wb['Sheet1']['K'+str(ROW+7)]='USA'
            wb['Sheet1']['D1']='FOUO' 
        elif df.iloc[ROW]['Discussion Portion Mark']== 'U':
        ...
        if df.iloc[ROW]['Recomendation Portion Mark']=='(U//FOUO)':
            wb['Sheet1']['N'+str(ROW+7)]='FOUO' 
            wb['Sheet1']['O'+str(ROW+7)]='USA'
            wb['Sheet1']['D1']='FOUO'
        elif df.iloc[ROW]['Recomendation Portion Mark']=='U':
            wb['Sheet1']['N'+str(ROW+7)]='FOUO'                       # does Recommendation portion marking
        ...
        if df.iloc[ROW]['WfF PM']== '(U//FOUO)':
            wb['Sheet1']['R'+str(ROW+7)]='FOUO' 
            wb['Sheet1']['S'+str(ROW+7)]='USA'
            wb['Sheet1']['D1']='FOUO' 
        elif df.iloc[ROW]['DOTMLPF Portion Mark']== '(U//FOUO)':
            wb['Sheet1']['R'+str(ROW+7)]='FOUO' 
            wb['Sheet1']['S'+str(ROW+7)]='USA'
            wb['Sheet1']['D1']='FOUO'
        elif df.iloc[ROW]['AWfC PM']=='(U//FOUO)':
            wb['Sheet1']['R'+str(ROW+7)]='FOUO' 
            wb['Sheet1']['S'+str(ROW+7)]='USA'
            wb['Sheet1']['D1']='FOUO'
        else:
        ...
        if df.iloc[ROW]['Mission Statement PM']== '(U//FOUO)':
            wb['Sheet1']['Z'+str(ROW+7)]='FOUO' 
            wb['Sheet1']['AA'+str(ROW+7)]='USA'
            wb['Sheet1']['D1']='FOUO' 
        elif df.iloc[ROW]['Mission Statement PM']=='U': 
            wb['Sheet1']['Z'+str(ROW+7)]=''            #mission statement portion marking
        ...
        if 'AWG Unit' in df:
            wb['Sheet1']['AI'+ str(ROW+7)] = df.iloc[0]['AWG Unit']       # adds author contact info 
        ROW = ROW-1 #restart while loop next observation     
############################ save the excel file   ###################################################################################
#wb.save(filename[:-4]+'.xlsx')
#wb['Sheet1'].add_table(TABLE) #add the new table on top of all the data
wb.save(XLfilename)
